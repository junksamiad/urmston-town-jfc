...
const CONFIG = {
  ...
  timeout: 120000  // 2 minutes for GitHub Actions
};
...
async function sendToAPI(fixtures) {
  ...
  try {
    ...
    if (typeof responseData === 'string') {
      // Extract just the JSON part if PHP code is mixed in
      const jsonMatch = responseData.match(/\{.*\}/);
      if (jsonMatch) {
        try {
          responseData = JSON.parse(jsonMatch[0]);
        } catch (parseError) {
          log('Failed to parse JSON response:', parseError.message);
    ...
    try {
      ...
      if (CONFIG.isTest) {
        ...
        if (result && result.stats) {
          ...
          log('API response received but stats unavailable');
        }
      }
